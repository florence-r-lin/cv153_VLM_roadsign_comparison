...
from config import GEMMA4_MODEL_ID, HF_CACHE_DIR, PROCESSED_DIR, PREDS_DIR, MAX_NEW_TOKENS, PROMPTS
...
def load_model():
    print(f"Loading {GEMMA4_MODEL_ID} ...")
    processor = AutoProcessor.from_pretrained(GEMMA4_MODEL_ID, cache_dir=HF_CACHE_DIR)
    model = _MODEL_CLS.from_pretrained(
        GEMMA4_MODEL_ID, device_map="auto", torch_dtype=torch.bfloat16, cache_dir=HF_CACHE_DIR,
    ).eval()
